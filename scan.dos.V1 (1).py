...
st.title("🦴 SpineScan V3 — Rasterstereography Inspired (Revopoint)")
...
if ply:
    ...
    if spine.shape[0] < 20:
        st.error("Midline impossible à extraire")
        st.stop()
    ...
    st.write(f"Flèche lombaire: **{fl:.2f} cm** → {'Normale' if fl_norm else 'Hors norme'}")
    st.write(f"Lordose: **{lord:.1f}°** → {'Normale' if lord_norm else 'Hors norme'}")
    st.write(f"Cyphose: **{kyph:.1f}°** → {'Normale' if kyph_norm else 'Hors norme'}")

    st.write(f"Fiabilité surface: {surface_conf*100:.0f}%")
